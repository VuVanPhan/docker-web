<?php
	namespace Maan\Search\Controllers\Index;

	use Maan\Framework\Model\DbConnect as DbConnect;
	use Maan\Framework\Controllers\Redirect\Redirect as Redirect;
	/**
	 * 
	 */
	class Index extends Redirect
	{
		private $_conn;
		public function __construct()
		{
			$this->_conn = new DbConnect();
			$this->execute();
		}

		public function execute() {
			$data = trim( htmlspecialchars( $_POST["key-search"] ) );
			$result = [];
			try {
				if ( isset( $data ) && !is_null( $data  ) && !empty( $data ) ) {

					$result["success"] = '';
				} else $result["error"] = "No data result";
				echo json_encode($result);
				return true;//$this->redirect("*/*/search/result");;
			} catch (PDOException $e) {
				echo "Execute failed: " . $e->getMessage();
			}
			

			// $requestUrl = trim( htmlentities( $_SERVER["REDIRECT_URL"] ) );
			// $dataUrl = explode( "/", $requestUrl );
			// $id = trim( $dataUrl[4] );
			// if ( isset( $id ) && !is_null( $id ) && !empty( $id ) ) {
			// 	if ( strtoupper($id) == 'ID' ) {
			// 		$conn = $this->_conn->connect();
			// 		$id = $dataUrl[5];
			// 		$sql = "SELECT * FROM products WHERE id = '". $id ."'";
			// 		foreach ($conn->query($sql) as $row) {
			// 			$data = $this->arraysToObject( $row );
			// 		}
			// 		// $data = $this->arraysToObject( $conn->query($sql) );
			// 	} else $this->redirect("*/*/error");
			// } else $this->redirect("*/*/error");
			// require_once(VENDOR_MAAN_PATH.'Product/View/templates/Frontend/Index.phtml');
		}
	}
?>